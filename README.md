...
Создан экспериментальный скрипт, позволяющий провести все шаги установки автоматически.
Данный скрипт предназначен для установки на станции, выполненные на основе **Raspberry Pi**.  
**Не тестировался** должным образом на **Raspberry Pi 3 Jessie** (нет в наличии).
...
```
И сохраните в ней полученные ранее конфигурационные файлы скриптов.
Выполните:
```
cd ~/source
...
./install.sh
```
Появится предупреждающее сообщение. Если вы готовы принять на себя все риски, связанные с автоматической установкой, нажмите "**y**".
Далее появятся запросы с требованием ввести предоставленные вам адрес, порт, логин и пароль.
После окончания процесса установки станция перезагрузится.

Скрипт сохраняет первоначальные копии конфигурационных файлов:
```
...
~/.bashrc.bak
```
Если после установки возникнут проблемы, это поможет в восстановлении работоспособности станции.

## Стандартная установка
Если вы не хотите использовать скрипт автоматической установки, переходите к данному разделу.
...
sudo reboot
```
После загрузки внести изменения в файл **live.jpg** с помощью встроенного графического редактора [GIMP](https://www.gimp.org/).
Повторно скопировать тестовый файл:
```
cp ~/source/ExScripts/Starvisor/live.jpg ~/RMS_data
```
Затем снова проверить страницу станции.
